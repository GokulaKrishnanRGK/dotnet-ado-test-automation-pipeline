...
There are two migration-related outputs with different roles:

- `Persistence/Migrations/` contains EF Core migration `.cs` files and the model snapshot. These files are the source of truth and are generated by `dotnet ef migrations add`.
- `Database/migrations/` contains generated idempotent `.sql` exports from those EF Core migrations. These are review/deployment artifacts for CI and Azure DevOps setup, not a second source of truth.

Developers should not hand-type full EF Core migration classes. Use the EF Core CLI to generate migration files, then review and deliberately edit the generated `Up` and `Down` methods when database behavior requires custom SQL such as stored procedures/functions.
